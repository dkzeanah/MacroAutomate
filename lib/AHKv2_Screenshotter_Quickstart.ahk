...
; Mouse-centered 100x100 capture WITHOUT cursor
^!p::
{
	CaptureMouseRegion(false)
	return
}

; Mouse-centered 100x100 capture WITH cursor
^!+p::
{
	CaptureMouseRegion(true)
	return
}
...
;#####################################################################################
CaptureMouseRegion(includeCursor := false)
{
	; Get current mouse position in screen coordinates
	MouseGetPos &mouseX, &mouseY

	; Define a 100x100 box centered on the mouse (50px in each direction)
	regionX := mouseX - 50
	regionY := mouseY - 50
	regionW := 100
	regionH := 100

	; Take the screenshot – with or without cursor
	if includeCursor
	{
		pBitmap := Gdip_BitmapFromScreenWithCursor(regionX, regionY, regionW, regionH)
	}
	else
	{
		; Standard bitmap from screen – cursor is NOT included by BitBlt
		pBitmap := Gdip_BitmapFromScreen(regionX "|" regionY "|" regionW "|" regionH)
	}

	; Build filename and save
	fileName := A_YYYY "-" A_MM "-" A_DD "-" A_Hour "-" A_Min "-" A_Sec "-MouseRegion.png"
...
;=====================================================================================
; Gdip_BitmapFromScreenWithCursor
;   Takes a  w x h  region starting at (x, y) and draws the mouse cursor into it.
;   Returns a GDI+ bitmap pointer like Gdip_BitmapFromScreen.
;=====================================================================================
Gdip_BitmapFromScreenWithCursor(x, y, w, h, Raster := "")
{
	...
	; Copy the screen region into our memory DC
	hhdc := GetDC()
	BitBlt(chdc, 0, 0, w, h, hhdc, x, y, Raster)
	ReleaseDC(hhdc)

	;-------------------------------------------
	; Overlay the current mouse cursor (if showing)
	;-------------------------------------------
	structSize := (A_PtrSize = 8) ? 24 : 20     ; CURSORINFO size
	CURSORINFO := Buffer(structSize, 0)
	NumPut("UInt", structSize, CURSORINFO, 0)   ; cbSize

	if DllCall("GetCursorInfo", "UPtr", CURSORINFO.Ptr)
	{
		flags := NumGet(CURSORINFO, 4, "UInt")   ; flags
		if (flags & 0x00000001)                 ; CURSOR_SHOWING = 0x00000001
		{
			hCursor := NumGet(CURSORINFO, 8, "UPtr")

			; Screen position of cursor
			pointOffset := 8 + A_PtrSize
			cursorX := NumGet(CURSORINFO, pointOffset, "Int")
			cursorY := NumGet(CURSORINFO, pointOffset + 4, "Int")

			; Convert to coordinates inside our captured region
			drawX := cursorX - x
			drawY := cursorY - y

			; Draw the cursor icon into the memory DC
			; DI_NORMAL = 0x0003 draws the icon using the system default size
			DllCall("DrawIconEx"
				, "UPtr", chdc
				, "Int",  drawX
				, "Int",  drawY
				, "UPtr", hCursor
				, "Int",  0            ; width (0 = default)
				, "Int",  0            ; height (0 = default)
				, "UInt", 0            ; istepIfAniCur
				, "UPtr", 0            ; hbrFlickerFreeDraw
